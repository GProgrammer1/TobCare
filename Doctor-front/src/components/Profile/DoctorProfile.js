import React, { useState, useEffect } from "react";
import { Star, User, MapPin, Hospital, Clipboard, CreditCard, Phone } from "lucide-react";
import "./DoctorProfile.css";
import medlicense from "../resources/medical-license.jpg";

const defaultProfileImage = 'data:image/png;base64,';

const DoctorProfile = () => {
  const [profile, setProfile] = useState();
  const [isEditing, setIsEditing] = useState(false);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");

  const handleEditSave = () => {
    if (isEditing) {
      // Save changes (for now, simply log the updated profile)
      console.log("Profile saved:", profile);
    }
    setIsEditing(!isEditing);
  };

  useEffect(() => {
    // Assume the doctor's email is stored in localStorage after login
    const email = localStorage.getItem("doctorEmail");
    console.log("Email: ", email);
    
    const token = localStorage.getItem("doctorToken");
    console.log("token",token);
    
    fetch(
      `https://ednisgva11.execute-api.us-east-1.amazonaws.com/dev/doctor_profile?email=${email}`,
      {
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`,
        },
      }
    )
      .then((res) => res.json())
      .then((data) => {
        let profileData = data;
        console.log("data:", profileData);
        
        if (data) {
          try {
            profileData = JSON.parse(data);
          } catch (e) {
            console.error("Error parsing response body", e);
          }
        }
        if (profileData.message) {
          setError(profileData.message);
        } else {
          setProfile(profileData);
        }
      })
      .catch((err) => {
        setError("Error fetching profile data: " + err.message);
      })
      .finally(() => {
        setLoading(false);
      });
  }, []);

  if (loading) return <div>Loading profile...</div>;
  if (error) return <div>Error: {error}</div>;

  // Destructure profile data
  const { name, specialty, bio, city, address, consultation_fee, username, follow_up_fee, phone_number } = profile;

  return (
    <div className="profilepage">
      <div className="doctor-profile-container">
        {/* Profile Header */}
        <div className="doctor-profile-header">
          <img
            src={defaultProfileImage}
            alt="Doctor"
            className="doctor-profile-pic"
          />
          <div className="doctor-info">
            {isEditing ? (
              <input
                type="text"
                value={name}
                onChange={(e) => setProfile({ ...profile, name: e.target.value })}
                className="edit-input"
              />
            ) : (
              <h2 className="doctor-name">{name}</h2>
            )}
            {isEditing ? (
              <input
                type="text"
                value={specialty}
                onChange={(e) =>
                  setProfile({ ...profile, specialty: e.target.value })
                }
                className="edit-input"
              />
            ) : (
              <p className="doctor-specialty">{specialty}</p>
            )}
            <p className="doctor-rating">
              {[...Array(5)].map((_, index) => (
                <Star
                  key={index}
                  className="rating-icon"
                  fill={index < 4 ? "gold" : "none"}
                  stroke="gold"
                />
              ))}
              <span className="review-count">(Reviews)</span>
            </p>
            <button onClick={handleEditSave} className="editprofile">
              {isEditing ? "Save" : "Edit Profile"}
            </button>
          </div>
        </div>

        <div className="drvision">
          {/* Profile Information */}
          <div className="doctor-profile-info">
            <div className="info-item">
              <Clipboard className="info-icon" />
              {isEditing ? (
                <input
                  type="text"
                  value={bio}
                  onChange={(e) =>
                    setProfile({ ...profile, bio: e.target.value })
                  }
                  className="edit-input"
                />
              ) : (
                <span className="info-text">Bio: {bio}</span>
              )}
            </div>
            <div className="info-item">
              <MapPin className="info-icon" />
              {isEditing ? (
                <input
                  type="text"
                  value={city}
                  onChange={(e) =>
                    setProfile({ ...profile, city: e.target.value })
                  }
                  className="edit-input"
                />
              ) : (
                <span className="info-text">City: {city}</span>
              )}
            </div>
            <div className="info-item">
              <User className="info-icon" />
              {isEditing ? (
                <input
                  type="text"
                  value={address}
                  onChange={(e) =>
                    setProfile({ ...profile, address: e.target.value })
                  }
                  className="edit-input"
                />
              ) : (
                <span className="info-text">Address: {address}</span>
              )}
            </div>
            <div className="info-item">
              <User className="info-icon" />
              {isEditing ? (
                <input
                  type="text"
                  value={username || ""}
                  onChange={(e) =>
                    setProfile({ ...profile, username: e.target.value })
                  }
                  className="edit-input"
                />
              ) : (
                <span className="info-text">Username: {username}</span>
              )}
            </div>
            <div className="info-item">
              <CreditCard className="info-icon" />
              {isEditing ? (
                <input
                  type="text"
                  value={consultation_fee || ""}
                  onChange={(e) =>
                    setProfile({ ...profile, consultation_fee: e.target.value })
                  }
                  className="edit-input"
                />
              ) : (
                <span className="info-text">
                  Consultation Fees: {consultation_fee}
                </span>
              )}
            </div>
            <div className="info-item">
              <CreditCard className="info-icon" />
              {isEditing ? (
                <input
                  type="text"
                  value={follow_up_fee || ""}
                  onChange={(e) =>
                    setProfile({ ...profile, follow_up_fee: e.target.value })
                  }
                  className="edit-input"
                />
              ) : (
                <span className="info-text">
                  Follow-up Fee: {follow_up_fee}
                </span>
              )}
            </div>
            <div className="info-item">
              <Phone className="info-icon" />
              {isEditing ? (
                <input
                  type="text"
                  value={phone_number || ""}
                  onChange={(e) =>
                    setProfile({ ...profile, phone_number: e.target.value })
                  }
                  className="edit-input"
                />
              ) : (
                <span className="info-text">
                  Phone Number: {phone_number}
                </span>
              )}
            </div>
          </div>

          <div className="dr-media">
            <img src={medlicense} alt="medical-license" className="profile-image-right" />
          </div>
        </div>
      </div>
    </div>
  );
};

export default DoctorProfile;
