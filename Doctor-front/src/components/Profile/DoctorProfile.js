import React from "react";
import { Star, User, MapPin, Hospital, Clipboard, DollarSign, CreditCard } from "lucide-react"; // Icons
import "./DoctorProfile.css"

const defaultProfileImage = 'data:image/png;base64,';
const DoctorProfile = ({ }) => {
    const maxStars = 5;
    const rating = 4; // Change this number to test different ratings
    const reviewCount = 178;

  return (
    <div className="profilepage">
    <div className="doctor-profile-container">
      {/* Profile Section */}
      <div className="doctor-profile-header">
        {/* Profile Picture */}
        <img 
        src={defaultProfileImage} 
        alt="Doctor" 
        className="doctor-profile-pic" 
        />
        {/* Doctor's Info */}
        <div className="doctor-info">
          <h2 className="doctor-name">Samira Baydoun</h2>
          <p className="doctor-specialty">Dentist</p>
          <p className="doctor-rating">
            {[...Array(maxStars)].map((_, index) => (
            <Star
                key={index}
                className="rating-icon"
                fill={index < rating ? "gold" : "none"}
                stroke="gold"
            />
            ))}
            <span className="review-count">({reviewCount} Reviews)</span>
           </p>
           <button value={"Edit Profile"} className="editprofile" >Edit Profile</button>
        </div>
      </div>

    {/* Information Boxes */}
    <div className="doctor-profile-info">
        <div className="info-item">
          <Clipboard className="info-icon" />
          <span className="info-text">Bio: Experienced dentist with 10 years of practice specializing in cosmetic dentistry</span>
        </div>
        <div className="info-item">
          <MapPin className="info-icon" />
          <span className="info-text">City: Beirut</span>
        </div>
        <div className="info-item">
          <User className="info-icon" />
          <span className="info-text">Address: 123 Main Street, Downtown</span>
        </div>
        <div className="info-item">
          <Hospital className="info-icon" />
          <span className="info-text">Current Hospital: Beirut Medical Center</span>
        </div>
        <div className="info-item">
          <Hospital className="info-icon" />
          <span className="info-text">Clinics: Smile Clinic, Dental Care Center</span>
        </div>
        <div className="info-item">
          <CreditCard className="info-icon" />
          <span className="info-text">Consultation Fees: $100 - $200</span>
        </div>
      </div>
    </div>
    </div>
  );
};

export default DoctorProfile;
